...
if __name__ == "__main__":
    ...
    start = time.time()
    noise='hybrid'
    directory = os.path.join("/data/dadwal/watermarkingTraj/data/All_results/TrajGuard/our_data/noise_traj/cropfromlast/")
    for root,dirs,files in os.walk(directory):
        for file in files:
            df=pd.read_csv("/data/dadwal/watermarkingTraj/data/All_results/TrajGuard/our_data/noise_traj/cropfromlast/"+file,header=None)
            ...
            noisy_trace = add_white_noise(df.values)
           ...
           # print(traj_folder)
            ...
            final_df=final_df[['trip_id','time','c_w','c_w_long']]
            final_df.to_csv( '../'+config['global']['global_fol']+config['global']['technique']+'/our_data/noise_traj/'+noise+'/'+str(trip_id)+'.csv',index=False,header=False)
