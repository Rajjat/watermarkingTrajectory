...
def interpolate(df) :
    ...
    for i in index1:
        ...
        line = pd.DataFrame({"trip_id": df.trip_id.values[0], 'longitude':0.0, 'latitude':0.0, 'time':temporal, 'Distance':0,'time_diff':0, 'cum_sum':0, 'watermarked_lat':0.0,'watermarked_long':0.0,'dist':0,'c_w':noised_lat,'c_w_long':
                            noised_long}, index=[i])
