...
def watermarkExtract(watermark,trip_id):
    ...
    for noise in noises:
        ...
        if trip_id==df_wc['trip_id'][0]:
            ...
            corr_watermark = np.corrcoef(W_star.flatten(), watermark.flatten())
            df_wc['dist'] = df_wc.apply(lambda row: haversine(row['longitude'], row['c_w'],
                                                                row['longitude'], row['watermarked_latitude']), axis=1)
...
if __name__ == "__main__":
    ...
    for trip_id in trip_idSeries:
        ...
        df=watermarkExtract(watermark1,trip_id)
        df_concat1 = pd.concat([df_concat1, df])
    df_concat1.to_csv( '../'+config['global']['global_fol'] + config['global']['technique'] + '/256_len/watermark_cor_withNoises_double_embed.csv',
            index=False)
