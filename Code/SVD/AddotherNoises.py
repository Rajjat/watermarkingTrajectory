...
def _wat_noise_with_signal_to_noise_ratio(signal, signal_to_noise_ratio, indices=None):
    ...
    for root,dirs,files in os.walk(directory):
        for file in files:
            ...
            for noise in noises:
                ...
                compare_dist=df[['watermarked_lat','watermarked_long','time','latitude','longitude']]
                frames=[df_noisy,compare_dist]
                final_df=pd.concat(frames, axis=1)
                final_df['dist']=final_df.apply(lambda row: haversine_np(row['watermarked_long'],row['watermarked_lat'],
